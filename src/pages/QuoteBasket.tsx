...
const QuoteBasket = () => {
  ...
  const handleRemoveItem = (id: string) => {
    ...
    toast({
      title: "Item Removed",
      description: "Quote item has been removed from your basket.",
    });
  };
  ...
  const handleSubmitAllQuotes = () => {
    if (state.items.length === 0) {
      toast({
        title: "No Items",
        description: "Add some items to your quote basket first.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Quote Basket</h1>
          <p className="text-muted-foreground">
            {state.totalItems} item{state.totalItems !== 1 ? 's' : ''} in your quote basket
          </p>
        </div>
      ...
      {state.totalItems === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">Your quote basket is empty</h3>
            <p className="text-muted-foreground mb-4">
              Add products to your quote basket to request bulk pricing
            </p>
            <Button asChild>
              <a href="/products">
